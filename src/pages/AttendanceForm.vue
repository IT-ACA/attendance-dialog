  ...
  <v-container style="margin-top: 50px;">
    <v-flex xs12>
      <v-row>
        ...
        <v-spacer></v-spacer>
        <!-- <pre>{{attendance}}</pre> -->
        <v-btn tile outlined to="/calendar" color="primary">
          ...
          >
            ...
            <div class="mdl-card__title">
              <h2 class="mdl-card__title-text">
                {{ eventData.name }} - Group {{ eventData.groupNo }}
              </h2>
            </div>
          ...
          <template v-if="eventData.grpScheduleID">
            <thead>
              <tr>
                ...
                <th class="text-left">Remarks <small>(Optional)</small></th>
              </tr>
            </thead>
            <tbody>
    ...
    <modal
      :active="showModal"
      text="Attendance Submitted. Thank you!"
      type="info"
  ...
  methods: {
    ...
    async createStudentAttendance() {
      const attendanceRecord = {
        fname:
          privateClassData[0]["portalData"]["Student"][0][
            "Student::First Name"
          ],
        sname:
          privateClassData[0]["portalData"]["Student"][0]["Student::Last Name"],
        sID:
          privateClassData[0]["portalData"]["Student"][0][
            "Student::_kp_student_id"
          ],
    ...
    updateAttendance(index) {
      if (index === 0) {
        ...
        console.log(
          "INDEX of group student attendance record : ",
  ...
  watch: {
    selectedOption: function() {
      if (this.selectedOption === "Attendance Form") {
        ...
        this.selectedOption !== "Attendance Form"
      ) {
